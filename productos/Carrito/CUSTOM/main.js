...
galeriaConts.forEach(galeriaCont => {
    galeriaCont.addEventListener('click', event => {
        ...
        imgCont.style.backgroundImage = `url('../../IMG/placas personalizadas (${event.target.id}).jpeg')`
    })
})
...
ModalgaleriaConts.forEach(ModalgaleriaCont => {
    ModalgaleriaCont.addEventListener('click', event => {
        ...
        ModalimgCont.style.backgroundImage = `url('../../IMG/placas personalizadas (${event.target.id.slice(-1)}).jpeg')`
    })
})
...
function drawProductInModal (){
    productCont.innerHTML = `
        <div class="cart-modal-det-cont">
            <img src="../../IMG/placas personalizadas (1).jpeg" class="cart-modal-img" alt="">
            <div>
                <p class="cart-modal-producto">PLACAS 100% CUSTOM</p>
                <p class="cart-modal-precio">$85.900 x1 <span>$85.900</span></p>
            </div>
            <i class="fa-solid fa-trash" id="cart-modal-delete"></i>
        </div>
        <a class="cart-modal-pagar" href="https://wa.link/1ff0q0">COMPRAR</a>`;
    deleteProduct();
    let priceModal = document.querySelector('.cart-modal-precio');
    priceModal.innerHTML = `$85.900 x${lastValue} <span>$${lastValue*85900}</span>`;
}
...
function NextImg(imgCont) {
    ...
    imgCont.style.backgroundImage = `url('../../IMG/placas personalizadas (${imgIndex}).jpeg')`
}
