...
galeriaConts.forEach(galeriaCont => {
    galeriaCont.addEventListener('click', event => {
        ...
        imgCont.style.backgroundImage = `url('../../IMG/led-${event.target.id}.webp')`
    })
})
...
ModalgaleriaConts.forEach(ModalgaleriaCont => {
    ModalgaleriaCont.addEventListener('click', event => {
        ...
        ModalimgCont.style.backgroundImage = `url('../../IMG/led-${event.target.id.slice(-1)}.webp')`
    })
})
...
function drawProductInModal (){
    productCont.innerHTML = `
        <div class="cart-modal-det-cont">
            <img src="../../IMG/led-1.webp" class="cart-modal-img" alt="">
            <div>
                <p class="cart-modal-producto">COLLARES LED</p>
                <p class="cart-modal-precio">$32.500 x1 <span>$32.500</span></p>
            </div>
            <i class="fa-solid fa-trash" id="cart-modal-delete"></i>
        </div>
        <a class="cart-modal-pagar" href="https://wa.link/oj21u8">COMPRAR</a>`;
    deleteProduct();
...
function NextImg(imgCont) {
    ...
    imgCont.style.backgroundImage = `url('../../IMG/led-${imgIndex}.webp')`
}
