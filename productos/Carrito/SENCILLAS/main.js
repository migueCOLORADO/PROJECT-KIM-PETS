...
galeriaConts.forEach(galeriaCont => {
    galeriaCont.addEventListener('click', event => {
        ...
        imgCont.style.backgroundImage = `url('../../IMG/placa-${event.target.id}.webp')`
    })
})
...
ModalgaleriaConts.forEach(ModalgaleriaCont => {
    ModalgaleriaCont.addEventListener('click', event => {
        ...
        ModalimgCont.style.backgroundImage = `url('../../IMG/placa-${event.target.id.slice(-1)}.webp')`
    })
})
...
function drawProductInModal (){
    productCont.innerHTML = `
        <div class="cart-modal-det-cont">
            <img src="../../IMG/placa-1.webp" class="cart-modal-img" alt="">
            <div>
                <p class="cart-modal-producto">PLACAS SENCILLAS</p>
                <p class="cart-modal-precio">$28.900 x1 <span>$28.900</span></p>
            </div>
            <i class="fa-solid fa-trash" id="cart-modal-delete"></i>
        </div>
        <a class="cart-modal-pagar" href="https://wa.link/h2qfji">COMPRAR</a>`;
    deleteProduct();
    let priceModal = document.querySelector('.cart-modal-precio');
    priceModal.innerHTML = `$28.900 x${lastValue} <span>$${lastValue*28900}</span>`;
}
...
function NextImg(imgCont) {
    ...
    imgCont.style.backgroundImage = `url('../../IMG/placa-${imgIndex}.webp')`
}
