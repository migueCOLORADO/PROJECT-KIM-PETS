...
galeriaConts.forEach(galeriaCont => {
    galeriaCont.addEventListener('click', event => {
        ...
        imgCont.style.backgroundImage = `url('../../IMG/collares-${event.target.id}.jpg')`
    })
})
...
ModalgaleriaConts.forEach(ModalgaleriaCont => {
    ModalgaleriaCont.addEventListener('click', event => {
        ...
        ModalimgCont.style.backgroundImage = `url('../../IMG/collares-${event.target.id.slice(-1)}.jpg')`
    })
})
...
function drawProductInModal (){
    productCont.innerHTML = `
        <div class="cart-modal-det-cont">
            <img src="../../IMG/collares-1.jpg" class="cart-modal-img" alt="">
            <div>
                <p class="cart-modal-producto">COLLARES & CORREAS</p>
                <p class="cart-modal-precio">$31.500 x1 <span>$31.500</span></p>
            </div>
            <i class="fa-solid fa-trash" id="cart-modal-delete"></i>
        </div>
        <a href="https://wa.link/ay8cin" class="cart-modal-pagar">COMPRAR</a>`;
    deleteProduct();
    let priceModal = document.querySelector('.cart-modal-precio');
    priceModal.innerHTML = `$31.500 x${lastValue} <span>$${lastValue*31500}</span>`;
}
...
function NextImg(imgCont) {
    ...
    imgCont.style.backgroundImage = `url('../../IMG/collares-${imgIndex}.jpg')`
}
